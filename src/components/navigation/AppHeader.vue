  ...
  <header class="app-header">
    ...
    <div class="popover-anchor">
      ...
      <section v-if="openPopover === 'notification'" class="header-popover notification-popover">
        <header><h2>새 알림 3</h2><RouterLink to="/notifications" @click="openPopover = ''">알림함 들어가기</RouterLink></header>
...
<style scoped>
.app-header { position: sticky; z-index: 40; top: 0; display: flex; align-items: center; gap: 10px; min-height: var(--header-height); padding: 17px clamp(30px, 4vw, 62px); background: rgb(251 252 255 / 94%); backdrop-filter: blur(10px); }
...
.header-popover > header { display: flex; align-items: center; justify-content: space-between; padding-bottom: 12px; border-bottom: 1px solid var(--border); }
...
.notification-popover > header { padding-bottom: 13px; }
.notification-popover h2 { color: #242424; font-size: 22px; }
.notification-popover header a { font-size: 12px; }
.notification-popover > a { grid-template-columns: 8px minmax(0, 1fr) 34px; min-height: 73px; padding: 12px 14px; border-radius: 12px; }
.notification-popover > a:first-of-type { margin-top: 16px; background: var(--primary-soft); }
.notification-popover i { align-self: start; margin-top: 5px; color: var(--danger); font-size: 16px; line-height: 1; }
.notification-popover strong { font-size: 14px; }
.notification-popover small { font-size: 11px; line-height: 1.5; }
.notification-popover time { align-self: start; padding-top: 3px; color: #777; text-align: right; font-size: 10px; }

@media (max-width: 767px) {
  .app-header { min-height: 58px; padding: 10px 14px; border-bottom: 1px solid #e7e7e7; }
