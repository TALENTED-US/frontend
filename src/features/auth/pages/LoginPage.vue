...
<template>
  <div class="login-page">
    <aside class="login-visual desktop-only">
      <BrandLogo />
      <div><small>취업 준비, 막막하지 않게</small><h2>내 재정으로 취업 준비를 설계하세요.</h2><p>수입·지출·청년 정책을 함께 살펴보고 목표 취업일까지의 계획을 보여드려요.</p></div>
    </aside>

    <main class="login-main">
      <BrandLogo class="mobile-only mobile-brand" />
      <section class="login-card">
        <div class="desktop-only"><small>WELCOME BACK</small><h1>다시 만나서 반가워요</h1><p>버티와 함께 취업 준비 계획을 이어가세요.</p></div>
        ...
        <p class="signup-link">아직 계정이 없으신가요? <RouterLink to="/auth/signup">회원가입</RouterLink></p>
      </section>
    </main>
  </div>
</template>
